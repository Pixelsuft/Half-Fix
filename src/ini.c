...
int parse_cfg(struct pc_settings* pc, char* data)
{
    ...
    if (!vgabios) {
        fprintf(stderr, "No VGABIOS!\n");
        goto fail;
    }

    if (load_file(&pc->bios, bios)) {
        fprintf(stderr, "Can't to load BIOS image\n");
        goto fail;
    }
    if (load_file(&pc->vgabios, vgabios)) {
        fprintf(stderr, "Can't to load VGABIOS image\n");
        goto fail;
    }
    ...
    pc->pci_enabled = get_field_int(global, "pci", 0);
    pc->acpi_enabled = get_field_int(global, "acpi", 0);
    pc->apic_enabled = get_field_int(global, "apic", 0);
    pc->floppy_enabled = get_field_int(global, "floppy", 0);
    ...
    if (res) {
        fprintf(stderr, "Unable to load disk drive images\n");
        goto fail;
    }
    ...
    if (res) {
        fprintf(stderr, "Unable to load floppy drive images\n");
        goto fail;
    }
    ...
    // Get CPU information
    pc->cpu.cpuid_limit_winnt = get_field_int(global, "winnt_fix", 0);
