...
{
    char buffer[1000];
    sprintf(buffer, "Half Fix i386 Emulator - "
                    " [%dx%d] (%s)",
        cwidth, cheight,
        mouse_enabled ? "Press Escape to release mouse" : "Click to capture mouse");
...
static LRESULT CALLBACK display_callback(HWND hwnd, UINT msg, WPARAM wparam, LPARAM lparam)
{
    ...
    case WM_DESTROY:
        ...
        break;
    case WM_KEYDOWN:
        if(wparam==VK_LSHIFT)is_shift=1;
        else if(mouse_enabled && wparam==VK_ESCAPE)
         display_capture_mouse(0);
        else
        {
          if(is_shift)display_kbd_send_key(win32_to_scancode(VK_LSHIFT) | 0x80);
          display_kbd_send_key(win32_to_scancode(wparam));
        }
        printf("Key down!!\n");
        break;
    case WM_KEYUP:
        if(mouse_enabled && wparam==VK_ESCAPE)
         display_capture_mouse(0);
        else if(wparam==VK_LSHIFT)is_shift=0;
        printf("Key up!!\n");
        break;
    case WM_MOUSEMOVE:
        if (mouse_enabled) {
    ...
    case WM_COMMAND:
        switch (LOWORD(wparam)) {
          case MENU_EXIT:
              exit(0);
          case MENU_RESET:
              printf("Reboot requested, quiting now...\n");
              exit(0);
          ...
          case MENU_CTRLALTDEL: {
            display_kbd_send_key(0x1D | 0); // CTRL
            display_kbd_send_key(0xE038 | 0); // ALT
            display_kbd_send_key(0xE053 | 0); // DEL
            display_sleep(100);
            display_kbd_send_key(0x1D | 0x80); // CTRL
            display_kbd_send_key(0xE038 | 0x80); // ALT
            display_kbd_send_key(0xE053 | 0x80); // DEL
            break;
          }
          case MENU_ALTTAB: {
            display_kbd_send_key(0xE038 | 0); // ALT
            display_kbd_send_key(0x0F | 0); // TAB
            display_sleep(100);
            display_kbd_send_key(0xE038 | 0x80); // ALT
            display_kbd_send_key(0x0F | 0x80); // TAB
            break;
          }
          case MENU_ALTF4: {
            display_kbd_send_key(0xE038 | 0); // ALT
            display_kbd_send_key(VK_F4 - VK_F1 + 0x3B | 0); // F4
            display_sleep(100);
            display_kbd_send_key(0xE038 | 0x80); // ALT
            display_kbd_send_key(VK_F4 - VK_F1 + 0x3B | 0x80); // F4
            break;
          }
          case MENU_LOCKMOUSE: {
            display_capture_mouse(1);
...
void display_init(void)
{
    ...
    AppendMenu(file, MF_STRING, MENU_LOAD_STATE, "&Load State");
    AppendMenu(file, MF_STRING, MENU_SAVE_STATE, "&Save State");

    AppendMenu(machine, MF_STRING, MENU_EXIT, "&Turn Off");
    AppendMenu(machine, MF_STRING, MENU_RESET, "&Reset");

    AppendMenu(mouse, MF_STRING, MENU_LOCKMOUSE, "&Lock Mouse");

    AppendMenu(keys_s, MF_STRING, MENU_CTRLALTDEL, "&Ctrl + Alt + Del");
    AppendMenu(keys_s, MF_STRING, MENU_ALTTAB, "&Alt + Tab");
    AppendMenu(keys_s, MF_STRING, MENU_ALTF4, "&Alt + F4");

    AppendMenu(bar, MF_POPUP, file, "&File");
    AppendMenu(bar, MF_POPUP, machine, "&Machine");
    AppendMenu(bar, MF_POPUP, mouse, "&Mouse");
    AppendMenu(bar, MF_POPUP, keys_s, "&Keyboard");
    SetMenu(hWnd, bar);
